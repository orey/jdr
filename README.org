...
En effet, regardons la répartition de 3D6 en termes de probabilités. Quand nous lançons 3D6, nous avons 16 scores possibles.
...
La figure suivante compare cette répartition avec celle de 1D16 (ce dé n'existe pas, mais il montre une distribution équiprobable des différents nombres sur un lancer).

Certains nombres sont beaucoup plus difficiles à atteindre avec 3D6 qu'avec 1D16, notamment les très petits petits et les très grands nombres.

[[file:3D6.png]]

Si le but est de faire moins qu'un certain seuil (par exemple, un jeu de chance dans le cadre de Fighting Fantasy), avec une borne de 12, nous avons déjà 75% de chances de réussir, ce qui est très favorable.  

#+ATTR_HTML: :border 2 :rules all :frame border
| Seuil | % de chances sous seuil 3D16 | % de chance sous seuil avec 1D16 |
|-------+------------------------------+----------------------------------|
|     3 | 0,46                         | 6,25                             |
|     4 | 1,85                         | 12,5                             |
|     5 | 4,63                         | 18,75                            |
