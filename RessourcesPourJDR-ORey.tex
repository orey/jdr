...
\section{Liens}
\label{sec:org087f6ed}

\subsection{Sites de jeux en français}
\label{sec:orgf40287d}
...
2022 & \href{https://www.cortexrpg.com/compendium/explore-the-rules/}{Cortex} & Generic System &  & - & N & \textbf{Y}\\
2022 & \href{https://www.drivethrurpg.com/product/117563}{The Void} & Horror Sci-Fi & Interesting Cthulu Saga & - & N & \textbf{Y}\\
2022 & Troika! & Generic system & A reinterpretation of the \href{https://github.com/orey/jdr/tree/master/FightingFantasys-fr}{Fighting Fantasy} rules with funny elements & 4/5 & ? & \textbf{Y}\\
2022 & 1PG Star Legion & Sci-Fi & A sci-Fi small RPG & - & N & \textbf{Y}\\
...
\label{sec:org5bd5963}

\begin{enumerate}
\item Des PJ à base de chiffres
\label{sec:org8e64699}
...
\textbf{Des jeux pour vétérans ?}

Il faut une certaine expérience du JDR pour pouvoir définir des \emph{mots et expressions utiles} au jeu. En un sens, le JDR s'adresse implicitement à des vétérans, voire à des vétérans dans un mode jeu ironique (voire cynique). Ces derniers ont les codes du JDR : ils sauront quelle expression leur permet de "joueur comme à AD\&D".
...
\textbf{Incertitude autour des PJ et des PNJ}

Les mots ou phrases introduisent une \emph{incertitude autour des personnages}, incertitude à laquelle le MJ doit s'adapter.
...
Les mots et les phrases descriptives des PJ et PNJ  introduisent aussi une \emph{incertitude dans l'équilibre des PJ entre eux et avec les PNJ}, et donc une possibilité d'arbitraire pour les MJ. Il devient, en effet, complexe de quantifier les expressions pour équilibrer les personnages et les aventures.

\textbf{Favoriser les extravertis}

Les mots et les phrases \emph{favorisent les joueurs extravertis} qui pourront interpréter de manière libre un "cliché" alors que les introvertis seront desservis par des règles basées sur des mots et sur le besoin d'improviser oralement pour les interpréter. Dans les jeux classiques, les joueurs utilisent des échelles objectives, ce qui permet à tous d'avoir une échelle commune pour s'exprimer.

\textbf{Deux types de MJ}
...
\item La structuration du "backgound" des personnages
\label{sec:org399521a}
...
Beaucoup de jeux plus récents établissent une vraie \emph{dictature du background} en exigeant de le structurer de manière schématique voire caricaturale, en termes de jeu. Ainsi, on voit apparaître, en plus de la notion de "cliché" ou d'"archétype"  :
\begin{itemize}
\item Une certaine obsession pour les \emph{défauts des PJ}, souvent utiles pour gagner des points dans le processus de création, parfois obligatoires dans le processus, souvent en contrepartie des dons ;
...
\item Les playbooks : des caricatures de classes de PJ ?
\label{sec:org65dd8ac}
...
\item Des caricatures de PJ pour un petit univers ?
\label{sec:org206b832}
...
Cette \emph{réduction de l'univers de jeu} semble une caractéristique des jeux PbtA. Les PJ archétypaux évoluent dans un monde restreint, archétypal. Notons que cette caractéristique implique la multiplication des jeux, nous y reviendrons.
...
Si l'on examine finement des jeux comme \emph{Apocalypse World}, on se rend compte que les moves des PJ et du MC (Master of Ceremony, un genre de "MJ light et collaboratif") semblent se répondre et avoir fait l'objet de simulations par ordinateur. En contraignant les actions possibles des PJ, et en catégorisant les différentes actions possibles du MC, le jeu donne l'impression de mettre en place une \emph{chorégraphie} bien adaptée à certains jeux ou univers, mais au final très mécaniste, et dont les résultats ont été "optimisés" dans des simulateurs inaccessibles aux joueurs et au MJ.

Certes, le jeu possède cette \emph{dynamique organique} dont nous parlions auparavant, et donc fournit les effets promis. Mais cela est rendu possible au travers des enchaînements de moves entre PJ et MC, ces dernières ayant fait l'objet de simulations informatisées ayant été utilisées dans le game design.

Les moves ne sont donc pas des vraies compétences, mais plus, comme leur nom l'indique, des actions appelant d'autres moves. Le glissement sémantique est subtil, mais réel : dans une situation, l'utilisation d'un move appelle un autre move "compatible" en face, et donc restreint mécaniquement les possibilités de jeu, en favorisant des "modèles d'action" (\emph{action patterns}) archétypaux au genre considéré.

\subsubsection{Théâtre et jeu de plateau}
\label{sec:org9d33cf6}

Nous sommes donc devant une nouvelle catégorie de jeux ayant les caractéristiques suivantes.

\textbf{Les personnages sont des \emph{archétypes} (playbook) aux actions archétypales (move)}
...
Nous sommes donc dans une perspective plus \emph{théâtrale} que libre. Le canevas imposé aux joueurs semble plus dur que le canevas imposé par les anciens jeux.

\textbf{Le JDR est devenu un jeu de plateau}
...
\subsubsection{L'éternel retour}
\label{sec:orgf115c94}
...
\begin{itemize}
\item Créer un jeu optimisé pour un sous-genre, voire un sous-sous-genre ;
...
\item Contraindre les joueurs et le MJ à évoluer dans un narratif qui contraint les PJ dans une sandbox, dans un univers de possibles limités, où les moves ont été optimisés pour créer mécaniquement une dynamique de jeu organique.
\end{itemize}
...
\label{sec:org5eda55d}
\subsubsection{Hurlements (1989)}
\label{sec:org7225a6d}
...
\label{sec:org06a88b5}

\begin{enumerate}
\item Mécanisme de combat
\label{sec:org60dd0e8}
